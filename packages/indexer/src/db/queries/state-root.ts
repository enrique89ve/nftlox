...
//   2. Advancing asset_count and last_block_num atomically with the root.
//   3. Bootstrapping the root from a full table scan when the row is zero.
//
// Every call MUST receive a `txn` that already wraps the actual Asset mutation,
// so a crash between the Asset row change and the root update cannot desync the
...
// Guards the DB→hash boundary. Every AssetStateRow that feeds the state-root
...
export function parseAssetStateRow(row: Record<string, unknown>): AssetStateRow {
	const id = row.id;
	if (typeof id !== "string" || id.length === 0) {
		throw new Error(`AssetStateRow.id: expected non-empty string, got ${describeValue(id)}`);
	}
	const owner = row.owner;
	if (typeof owner !== "string" || owner.length === 0) {
		throw new Error(`AssetStateRow.owner: expected non-empty string, got ${describeValue(owner)} (id=${id})`);
	}
	...
	if (previousOwnerRaw === null) {
		...
		throw new Error(
			`AssetStateRow.previous_owner: expected null or non-empty string, got ${describeValue(previousOwnerRaw)} (id=${id})`,
		);
	}
	const ownerAction = row.owner_action;
	if (typeof ownerAction !== "string" || ownerAction.length === 0) {
		throw new Error(`AssetStateRow.owner_action: expected non-empty string, got ${describeValue(ownerAction)} (id=${id})`);
	}
	const ownerOperationId = row.owner_operation_id;
	if (typeof ownerOperationId !== "string" || ownerOperationId.length === 0) {
		throw new Error(
			`AssetStateRow.owner_operation_id: expected non-empty string, got ${describeValue(ownerOperationId)} (id=${id})`,
		);
	}
	...
	if (typeof ownerBlockRaw === "number") {
		...
		throw new Error(
			`AssetStateRow.owner_block_num: expected number|bigint|string, got ${describeValue(ownerBlockRaw)} (id=${id})`,
		);
	}
	if (!Number.isFinite(owner_block_num) || !Number.isInteger(owner_block_num) || owner_block_num < 0) {
		throw new Error(
			`AssetStateRow.owner_block_num: expected non-negative integer, got ${describeValue(ownerBlockRaw)} (id=${id})`,
		);
	}
	return {
		id,
...
export async function getStateMeta(txn: Queryable = sql): Promise<StateMetaRow> {
	const [row] = await txn`
		SELECT state_root, asset_count, last_block_num, updated_at, divergent_at_block
		FROM state_meta
	...
	if (!Number.isFinite(assetCount) || assetCount < 0) {
		throw new Error(`state_meta.asset_count invalid: ${row.asset_count}`);
	}
...
// assets table. Safe to re-run; always overwrites. Streams in pages so memory
...
export async function bootstrapStateRootFromFullScan(
	...
	async function* streamRows(): AsyncGenerator<AssetStateRow> {
		let lastId = "";
		for (;;) {
			...
			for (const row of page) {
				yield parseAssetStateRow(row as Record<string, unknown>);
			}
...
export async function flushStateRootBuffer(
	...
	await txn`
		UPDATE state_meta
		SET state_root = ${Buffer.from(root)},
		    asset_count = asset_count + ${countDelta},
