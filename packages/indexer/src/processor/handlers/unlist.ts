...
import { getNftForProcessing, updateNftListing } from "../../db/queries/nfts.ts";
...
export async function handleUnlist(op: ParsedOperation, txn: Queryable): Promise<void> {
	...
	await updateNftListing(nftId, null, null, txn);
	await insertHistoryEvent({
		nftId, collectionId: nft.collection_id, eventType: "unlist",
