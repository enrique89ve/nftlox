...
// Public commitment to the projected Asset ownership state. Any two indexers
...
// The hash itself is maintained incrementally inside the Asset mutation path
// (see db/queries/state-root.ts).
//
// Callers verify ownership against HafAH using the SPV fields exposed in the
// per-Asset routes; the state-root is a complementary tool for comparing whole
...
export const stateRootRoutes = new Elysia({ tags: ["Status"] })
	.get("/api/state-root", async () => {
		...
		response: t.Object({
			...
			asset_count: t.Integer({ minimum: 0 }),
			last_block_num: t.Integer({ minimum: 0 }),
			updated_at: t.String(),
		}),
		detail: {
			summary: "Projected-state commitment",
			description: "Returns the incremental XOR state-root over all Asset SPV fields (owner, previous_owner, owner_action, owner_operation_id, owner_block_num, id). Two indexers on the same stream converge to the same value; divergence is a single byte comparison.",
		},
	});
