...
describe("handleBuyCommitment", () => {
	test("projects pending_sale on a listed Asset", async () => {
		const assetId = fixtureAssetId("commit-a");
		...
		const op = commitmentOp({ assetId, listingId, listTxId, buyer: BUYER, buyTxHash, blockNum: 100_500 });
	...
	test("rejects when Asset is not found", async () => {
		const op = commitmentOp({
			assetId: fixtureAssetId("ghost"),
	...
	test("rejects listingId mismatch", async () => {
		const assetId = fixtureAssetId("commit-mismatch");
		const realListing = fixtureListingId("real");
		const realTx = fixtureHiveTxId("real");
		await insertListedInstance({ assetId, collectionId: COLLECTION_ID, seller: SELLER, listingId: realListing, listTxId: realTx });
	...
	test("rejects when buyer is the seller", async () => {
		const assetId = fixtureAssetId("commit-self");
		const listingId = fixtureListingId("self");
		const listTxId = fixtureHiveTxId("self");
		await insertListedInstance({ assetId, collectionId: COLLECTION_ID, seller: SELLER, listingId, listTxId });
		const op = commitmentOp({ assetId, listingId, listTxId, buyer: SELLER });
	...
	// expired listing, keeping the Asset in `pending_sale` and blocking
	...
	test("rejects a commitment when the listing is already expired", async () => {
		const assetId = fixtureAssetId("commit-expired-listing");
		...
		// And the Asset must remain in its pre-commitment state (listed, no
	...
	test("rejects a competing commitment while a reservation is active", async () => {
		const assetId = fixtureAssetId("commit-race");
		...
		await withTransaction((txn) =>
			handleBuyCommitment(
				commitmentOp({ assetId, listingId, listTxId, buyer: BUYER, blockNum: firstBlock }),
				txn,
			),
		);
	...
	test("overwrites an expired reservation", async () => {
		const assetId = fixtureAssetId("commit-expired");
	...
	test("sweep clears expired reservations", async () => {
		const assetId = fixtureAssetId("commit-sweep");
	...
	test("rejects once a single node holds MAX_ACTIVE_COMMITMENTS_PER_NODE reservations", async () => {
		...
		for (let i = 0; i <= MAX_ACTIVE_COMMITMENTS_PER_NODE; i++) {
			const assetId = fixtureAssetId(`cap-${i}`);
