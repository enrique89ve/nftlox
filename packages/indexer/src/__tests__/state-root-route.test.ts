...
// ----------------------------
// Public commitment to the projected Asset state. Two indexers processing the
...
// (parseAssetStateRow, flushStateRootBuffer, queueStateRootDelta, …) stay live
...
describe("GET /api/state-root", () => {
	test("returns the singleton commitment with SPV-friendly shape", async () => {
		...
		expect(body.asset_count).toBe(42);
	...
	test("reflects updates to the underlying meta between calls", async () => {
		...
		expect(body.asset_count).toBe(100);
