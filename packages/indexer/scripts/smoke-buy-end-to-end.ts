...
async function main(): Promise<void> {
  ...
  console.log(
    "[smoke] unsigned tx built; ref_block_num=",
    unsignedTx.transaction?.ref_block_num,
  );
  ...
  console.log(
    "[smoke] buyer signed. sig prefix=",
    buyerSig.slice(0, 16),
    "...",
  );

  const body = {
    transaction: { ...unsignedTx.transaction, signatures: [buyerSig] },
  };
  ...
  console.log(
    `[smoke] ✓ Buy settled. tx_id=${json.txId} commitment_op_tx_id=${json.commitmentOpTxId}`,
  );
}
