...
- `asset_approve_all` stores `owner = op.signer`; the payload does not contain an owner field.
- `asset_transfer_from` accepts `from` in the payload, but it still requires `op.signer` to match an instance or collection approval from that owner.
...
`assets.owner` is the current owner. It changes only through ownership-changing actions.
...
| `asset_transfer_from` | Moves ownership through an instance or collection approval. |
| `buy` | Moves ownership after marketplace settlement. |
| `list`, `unlist` | Do not change owner. |
| `asset_lend`, `asset_return` | Do not change owner. They change use/custody state only. |
...
`asset_approve_all` is collection-scoped transfer authority for the current owner while the approval remains active.

| Invariant | Rule |
|---|---|
| C1 | `approved: true` requires the signer to own at least one Asset in the collection. |
| C2 | A collection approval covers all current Assets the owner has in that collection. |
| C3 | Future acquisitions are covered only while the approval row remains active. |
| C4 | The approval is automatically removed when the owner has zero Assets left in the collection. |
| C5 | The zero-holdings cleanup must run no matter which action emptied the owner: `transfer`, `buy`, `burn`, or `asset_transfer_from`. |
...
- `asset_approve` applies to one instance.
- It is cleared when ownership changes, the Asset is burned, bought, transferred, lent, or moved through `asset_transfer_from`.
...
| Invariant | Rule |
|---|---|
| L1 | Active listings block direct ownership and custody changes such as `transfer`, `asset_transfer_from`, `burn`, and `asset_lend`. |
...
| L5 | While a `buy_commitment` holds an Asset (`status = "pending_sale"`), `unlist`, `transfer`, `asset_transfer_from`, `burn`, `asset_lend`, and re-listing are all rejected. The Asset returns to `active` only when the matching `buy` settles or the commitment TTL expires. |
...
40 blocks ≈ 120 s @ 3 s/block) bounds how long an unlisted-but-reserved Asset
...
| D2 | `asset_approve` and `asset_approve_all` grant transfer authority, not mutable-data authority. |
...
| Invariant | Rule |
|---|---|
| N1 | The projected `state_root` is the incremental XOR over a fixed subset of every Asset's SPV fields (`owner`, `previous_owner`, `owner_action`, `owner_operation_id`, `owner_block_num`, `id`). The subset is consensus — adding or removing a field is a hardfork. |
| N2 | The state-root is updated inside the same transaction as the Asset mutation that changes it. Drift between the projection and its commitment is impossible by construction. |
