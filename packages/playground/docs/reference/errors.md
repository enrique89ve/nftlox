...
| `NODE_DIVERGENT` | 503 | This node's local integrity interlock was set after a verified local failure or explicit operator action. The gate refuses to co-sign buy or collection requests until an operator clears `state_meta.divergent_at_block` after audit. Route to a different indexer; this is **not** a transient retry. Peer checkpoint mismatches alone are advisory and do not produce this error. |
